...
export const DEFAULT_PROMPT_IMAGE_MODEL = 'image2' as const;
...
export function getImage2RelayParameterTips() {
  return [
    '前端只开放 output_format、output_compression、response_format、partial_images；quality 不再发送，由中转默认处理。背景固定 opaque，审核强度固定 low，stream 跟随 .env。',
    ...
    '编辑图的 file_id 不支持；本项目用上传、拖放、粘贴或外链导入参考图，再走 multipart image，并提供 mask 局部编辑 UI。',
  ];
}
