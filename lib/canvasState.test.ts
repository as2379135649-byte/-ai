...
import {
  ...
  normalizeImageGenerationMetrics,
  normalizeNodeDataWithAssets,
  pruneAssets,
  type PersistedCanvasState,
} from './canvasState';

test('normalizeImageGenerationMetrics keeps reported tokens and explicit missing output usage', () => {
  assert.deepEqual(
    normalizeImageGenerationMetrics({
      requestId: ' request-1 ',
      quality: 'medium',
      imageOutputTokens: 1056.9,
      inputTokens: 12,
      totalTokens: 1068,
    }),
    {
...
test('history snapshot preserves image model and drops removed Image2 output options', () => {
  const state: PersistedCanvasState = {
    nodes: [
      {
        id: 'prompt-1',
  ...
  assert.equal((history.nodes[0].data as any).image2Options, undefined);
  assert.equal((history.nodes[1].data as any).image2Options, undefined);
});
...
test('normalization allows five reference images and enforces that limit', () => {
  const normalized = normalizeNodeDataWithAssets(
    {
  ...
  assert.equal(normalized.data.referenceImageIds?.length, 5);
  assert.deepEqual(normalized.data.referenceImageIds?.slice(0, 4), ['a1', 'a2', 'a3', 'a4']);
  assert.equal(Object.keys(normalized.assets).length, 5);
});
