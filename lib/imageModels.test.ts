...
test('normalizeImageModel falls back to banana for unknown values', () => {
  ...
  assert.equal(DEFAULT_PROMPT_IMAGE_MODEL, 'image2');
...
test('normalizeImage2Options ignores fixed background, moderation, stream and quality overrides', () => {
  assert.deepEqual(
    normalizeImage2Options({
      background: 'opaque',
      moderation: 'auto',
...
test('getImage2RelayParameterTips documents relay-specific parameter differences', () => {
  ...
  assert.ok(tips.some((tip) => tip.includes('quality 不再发送')));
});
