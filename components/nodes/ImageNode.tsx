...
export function ImageNode({ id, data }: NodeProps<AppNode>) {
    ...
    useStore.setState((state) => ({
      edges: [...state.edges, {
        ...
        data: {
          ...
          referenceLink: true,
          referenceAssetId: data.imageAssetId,
        },
      }],
    }));
  };
  ...
  const handleMaskGenerate = async ({ prompt: maskPrompt, maskImage, sourceImage }: MaskGeneratePayload) => {
    ...
    try {
      let generationMetrics: AppNode['data']['generationMetrics'];
      const payload = buildImageMaskGenerationPayload({
        maskPrompt,
        maskImage,
        sourceImage,
        aspectRatio: data.aspectRatio || '1:1',
  ...
  if (!imageUrl && (data.isLoading || data.error)) {
    return (
      <div className="canvas-task-card relative group">
        <Handle type="target" position={Position.Left} className="canvas-connection-handle w-5 h-5 border-2 opacity-50 group-hover:opacity-100 transition-opacity" style={{background: '#9B70D0', borderColor: '#ffffff'}} />
        <GeneratingImagePlaceholder
          ...
          error={data.error}
        />
        <Handle type="source" position={Position.Right} className="canvas-connection-handle w-5 h-5 border-2 opacity-50 group-hover:opacity-100 transition-opacity" style={{background: '#5B9BD5', borderColor: '#ffffff'}} />
      </div>
    );
  }
  ...
  return (
    <div
      className="canvas-task-card rounded-2xl overflow-hidden transition-all group"
      style={{
        background: '#1D1A14',
    ...
    >
      <Handle type="target" position={Position.Left} className="canvas-connection-handle w-5 h-5 border-2 opacity-50 group-hover:opacity-100 transition-opacity" style={{background: '#9B70D0', borderColor: '#1D1A14'}} />

      <div className="relative min-w-[256px] min-h-[256px] flex items-center justify-center" style={{background: '#141210', borderRadius: '10px', overflow: 'hidden'}}>
        {imageUrl ? (
          <>
            <img
              src={imageUrl}
              alt={data.prompt || (data.generationMode === 'imported' ? '导入图片素材' : 'Generated image')}
        ...
        <div className="mt-3 px-2 pb-1 max-w-[512px] flex items-start justify-between gap-2">
          <div className="flex-1">
            <div className="mb-1 flex flex-wrap items-center gap-1.5">
              <div className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-medium" style={{background: 'rgba(242,193,78,0.08)', color: '#96836F'}}>
                {imageModelLabel}
              </div>
              {data.generationMetrics && (
                <div
                  ...
                  style={{
                    ...
                    color: data.generationMetrics.imageOutputTokens === null ? '#D9A441' : '#8FBCE6',
                  }}
                  title={`请求 ID：${data.generationMetrics.requestId}\n输入 token：${data.generationMetrics.inputTokens ?? '未返回'}\n总 token：${data.generationMetrics.totalTokens ?? '未返回'}`}
                >
                  {data.generationMetrics.quality === 'auto' ? '默认画质' : data.generationMetrics.quality} · 图片输出 {data.generationMetrics.imageOutputTokens === null
                    ? 'token 未返回'
                    : `${data.generationMetrics.imageOutputTokens.toLocaleString()} token`}
                </div>
              )}
            </div>
            ...
            {data.generationMetrics && (
              <p
                className="mt-1 max-w-[360px] truncate text-[9px]"
                style={{color: '#5C4E3E'}}
                title={data.generationMetrics.requestId}
              >
                请求 {data.generationMetrics.requestId}
              </p>
            )}
          </div>
          <button
      ...
      <Handle type="source" position={Position.Right} className="canvas-connection-handle w-5 h-5 border-2 opacity-50 group-hover:opacity-100 transition-opacity" style={{background: '#5B9BD5', borderColor: '#1D1A14'}} />
